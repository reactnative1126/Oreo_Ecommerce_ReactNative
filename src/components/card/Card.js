...
const Card = props => {
  ...
  return (
    <View
      {...attributes}
      style={StyleSheet.flatten([styles.container(theme), image && { padding: 0 }, containerStyle && containerStyle])}
    >
      <View style={StyleSheet.flatten([styles.wrapper, wrapperStyle && wrapperStyle])}>
          ...
          : title &&
            title.length && (
              <View>
                ...
                {!image && <Divider style={StyleSheet.flatten([styles.divider, dividerStyle && dividerStyle])} />}
              </View>
            )}
        ...
        {image && (
          <View style={imageWrapperStyle && imageWrapperStyle}>
            ...
            >
              {(featuredTitle || featuredSubtitle) && (
                <View style={styles.overlayContainer}>
                  {featuredTitle && (
                    <Text style={StyleSheet.flatten([styles.featuredTitle, featuredTitleStyle && featuredTitleStyle])}>
                      {featuredTitle}
                    </Text>
                  )}
                  {featuredSubtitle && (
            ...
            <View style={StyleSheet.flatten([{ padding: 10 }, wrapperStyle && wrapperStyle])}>{children}</View>
          </View>
        )}
