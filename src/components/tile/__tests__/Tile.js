...
describe('Tile component', () => {
  ...
  it('should render featured tile with icon', () => {
    const component = shallow(
      <Tile imageSrc={{ url: 'http://google.com' }} icon={{ name: 'play-circle', type: 'font-awesome' }} featured />
    );
  ...
  it('should render with active opacity', () => {
    const component = shallow(<Tile imageSrc={{ url: 'http://google.com' }} activeOpacity={0.9} />);
  ...
  it('should apply styles from theme', () => {
    ...
    expect(component.root.findByProps({ testID: 'tileTitle' }).props.children).toBe('Mary is friendly');
